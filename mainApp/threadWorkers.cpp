...
bool PartWorker::processPartsArchive(const QStringList &ldPartsDirs, const QString &comment = QString("")){
  ...
  logStatus() << "Archive parts...";
  logStatus() << QString("Archiving %1 parts to %2.").arg(comment,archiveFile);
...
//                                                                 tr("[%1 + %2] parts").arg(archivedPartCount).arg(partCount);
      ...
      logStatus() << tr("Archived %1 %2 from %3").arg(partCount).arg(breakdown).arg(foo.absolutePath());
  }

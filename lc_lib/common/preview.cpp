...
PiecePropertiesPreview::PiecePropertiesPreview()
{
    m_PieceInfo = NULL;
    m_RotateX = 60.0f;
    m_RotateZ = 225.0f;
...
void PiecePropertiesPreview::OnDraw()
{
    if (m_PieceInfo == NULL)
        return;
...
void PiecePropertiesPreview::SetCurrentPiece(PieceInfo *pInfo)
{
    MakeCurrent();
...
void PiecePropertiesPreview::SetDefaultPiece()
{
    lcPiecesLibrary* Library = lcGetPiecesLibrary();
    PieceInfo* Info = Library->FindPiece("4019", NULL, false);
...
void PiecePropertiesPreview::OnLeftButtonDown()
{
    if (m_Tracking == LC_TRACK_NONE)
    {
        m_DownX = mInputState.x;
...
void PiecePropertiesPreview::OnLeftButtonUp()
{
    if (m_Tracking == LC_TRACK_LEFT)
        m_Tracking = LC_TRACK_NONE;
}

void PiecePropertiesPreview::OnLeftButtonDoubleClick()
{
    m_AutoZoom = true;
    Redraw();
}

void PiecePropertiesPreview::OnRightButtonDown()
{
    if (m_Tracking == LC_TRACK_NONE)
    {
        m_DownX = mInputState.x;
...
void PiecePropertiesPreview::OnRightButtonUp()
{
    if (m_Tracking == LC_TRACK_RIGHT)
        m_Tracking = LC_TRACK_NONE;
}

void PiecePropertiesPreview::OnMouseMove()
{
    if (m_Tracking == LC_TRACK_LEFT)
    {
        // Rotate.
